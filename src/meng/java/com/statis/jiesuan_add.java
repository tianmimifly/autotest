...
public class jiesuan_add {
    ...
    public static  void  statis(){
        ...
        for (Object o :order_list){
            ...
            if(m2!=null) {
                ...
                Map  gameMap =MySqlHelper.getGamemallHelper().executeMap("select revenue_ration,game_id from t_game where outter_game_id='" + map.get("sdk_game_id") + "'");
                map.put("game_fc",String.valueOf(gameMap.get("revenue_ration")) );
                map.put("game_id",String.valueOf(gameMap.get("game_id")) );
                String zg_fc = MySqlHelper.getGamemallHelper().executeStr("select revenue_ration from t_mer_revenue_ration where game_id ='" + map.get("game_id") + "' and leader_uid='" +String.valueOf(m2.get("leader_uid"))  + "';");
                map.put("zg_fc", zg_fc);
                statisList.add(map);
            }
        }
    ...
    private static String statis_date="2018-01-19";
    private static void  statis_total(List<Map<String,String>> statisList){
        ...
        String strSQL="INSERT INTO `tmm_statis_js` (statis_date,`leader_uid`, `game_fc`, `member_uid`, `total`, `sdk_game_id`, `zg_fc`,grdjq) \n" +
                "VALUES ('"+statis_date+"','${leader_uid}', ${game_fc}, '${member_uid}', ${total}, '${sdk_game_id}', ${zg_fc},${grdjq});";
       // MySqlHelper.getTmmTestHelper().execute("set @statis_date='"+statis_date+"';");
        MySqlHelper.getTmmTestHelper().execute("delete from tmm_statis_js where statis_date='"+statis_date+"';");
        for (Map<String,String> m :listNew){
            ...
            MySqlHelper.getTmmTestHelper().execute(s);
